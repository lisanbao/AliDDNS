...
function_AliDDNS_UpdateRecord(){
    echo -e "${Msg_Info}正在检查记录状态……"
    if [ "${AliDDNS_RecordID}" = "" ]; then
        echo -e "${Msg_Info}正在获取RecordID……"
        AliDDNS_RecordID=`query_recordid | get_recordid`
    fi
    if [ "${AliDDNS_RecordID}" = "" ]; then
        echo -e "${Msg_Warning}未能获取到RecordID，DNS记录可能不存在，正在尝试新增记录……"
        echo -e "${Msg_Info}正在添加解析记录：$AliDDNS_SubDomainName.$AliDDNS_DomainName -> $AliDDNS_LocalIP ……"
        AliDDNS_AddResult=`add_record`
        AliDDNS_NewRecordID=`echo "$AliDDNS_AddResult" | grep -Eo '"RecordId":"[0-9]+"' | cut -d':' -f2 | tr -d '"'`
        if [ "${AliDDNS_NewRecordID}" != "" ]; then
            echo -e "\n${Msg_Success}DNS记录添加成功！RecordID：$AliDDNS_NewRecordID"
            echo -e "${Msg_Success}新的解析记录：$AliDDNS_SubDomainName.$AliDDNS_DomainName -> $AliDDNS_LocalIP"
            function_ServerChan_SuccessMsgPush
            exit 0
        else
            echo -e "\n${Msg_Fail}DNS记录添加失败！"
            echo -e "${Msg_Info}API返回信息：$AliDDNS_AddResult"
            exit 1
        fi
    else
        ...
        AliDDNS_UpdateResult=`update_record $AliDDNS_RecordID`
        AliDDNS_UpdateCheckID=`echo "$AliDDNS_UpdateResult" | grep -Eo '"RecordId":"[0-9]+"' | cut -d':' -f2 | tr -d '"'`
        if [ "${AliDDNS_UpdateCheckID}" != "" ]; then
            echo -e "\n${Msg_Success}DDNS记录更新成功，新的IP为：$AliDDNS_LocalIP"
            function_ServerChan_SuccessMsgPush
            exit 0
        else
            echo -e "\n${Msg_Fail}DDNS记录更新失败！"
            echo -e "${Msg_Info}API返回信息：$AliDDNS_UpdateResult"
            exit 1
        fi
    fi
}
